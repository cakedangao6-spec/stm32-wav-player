...
- `User/wav.c/.h`：WAV 头解析器，从 W25Q64 读取 RIFF/WAVE/fmt/data chunk，得到 PCM 参数。
- `User/audio_player.c/.h`：播放核心。把 8/16bit、单/双声道 PCM 转成 PWM 占空比，并用 TIM2 中断按采样率输出。
- `User/bsp_w25q64.c/.h`：W25Q64 的底层 SPI 读命令。
- `User/w25q64.c/.h`：应用层 W25Q64 包装，提供 JEDEC ID 检查和带边界检查的 `W25Q64_Read`。
- `User/bsp_usart.c/.h`：串口调试日志输出；本项目不需要从串口接收文件，但保留 USART 方便看错误原因。
- `User/ring_buffer.c/.h`：USART 驱动依赖的接收缓冲，在播放项目里不是主角。
...
2. 再看 `User/wav.c`，理解 WAV 文件为什么必须找到 `fmt ` 和 `data` 两个 chunk。
3. 然后看 `User/audio_player.c` 的 `AudioPlayer_Init`、`AudioPlayer_Start`、`AudioPlayer_Process`。
4. 最后看 `audio_player.c` 里的 `TIM2_IRQHandler`，这是实际把样本送到 PWM 的地方。
